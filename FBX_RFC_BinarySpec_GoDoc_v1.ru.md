...
Текущий `go-fbx` также определяет несовместимый **профиль расширения v1**
(при том же `Header.version = 1`) для более строгой семантики commit/recovery
и ленивого парсинга каталога. См.:
...
| 1 | HAS_BACKUP | Включена семантика fixed backup header (профиль расширения) |
| 2 | HAS_DIR_INDEX | Для открытия обязателен индекс каталога `IDX1` (профиль расширения) |
| 3 | HAS_REQUIRED_FEATURES | Активна маска required-features в `reserved[52:56]` (профиль расширения) |
| 4..31 | RESERVED | MUST быть 0, если extension-профиль не определяет иное |
...
- Профиль расширения `go-fbx` (с тем же `Header.version = 1`) дополнительно определяет:
  ...
  - правила конвертации/миграции без перепаковки payload.
- Детали профиля расширения описаны в:
  - `docs/V1_EXTENSION_SPEC.md`
  - `docs/MIGRATION.md`
